...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        User.IsInRole("LCLMTL\LCL_AE") Or User.IsInRole("LCLMTL\LCL_SA") Or User.IsInRole("LCLMTL\LCL_SIE") Then
            If Not (Request.QueryString("Status") = Nothing And Request.QueryString("Grouping") = Nothing _
                ...
                Dim strReq As String = "SELECT QuoteId AS [Quote #], Revision, CONVERT(varchar, [Date], 101) AS [Quote Date mm/dd/yyyy], CustomerID AS [Customer #], " & _
                                    ...
                                    "Item AS [Item #], ModelNumber AS [Model Number], ProductCode AS PC, Grouping, Status, " & _
                                    ...
                                    "FROM vOSREPORT WHERE " & Where() & " ORDER BY QuoteId, [Date], Item"
    ...
    Function Where() As String
        ...
        If Request.QueryString("Status") <> Nothing Then
            strWhere += "Status = '" & Request.QueryString("Status") & "' AND "
        End If
        If Request.QueryString("Grouping") <> Nothing Then
            strWhere += "Grouping = '" & Request.QueryString("Grouping") & "' AND "
        End If
        If Request.QueryString("LastName") <> Nothing Then
            strWhere += "LastName like '%" & Request.QueryString("LastName") & "%' AND "
        End If

        strWhere += "ModelNumber like '%" & Request.QueryString("ModelNumber") & "%'"
